...
```
Improved model:
We create two models: One which includes all first order terms (no interactions),
and one model with interactions between bill_depth and species, as in the initial
model. We the run an F-test to determine whether the more complex model improves on
the simpler model:
```{r}
# Fit improved model without interactions
penguin.model.improved0 <- lm(body_mass_g ~ 
                               flipper_length_mm 
                             + bill_length_mm
                             + bill_depth_mm 
                             + species
...
penguin.model.improved <- lm(body_mass_g ~ 
                              ...
                              * species
                              ...
                              data = Penguins)
# compare models:
anova(penguin.model.improved0, penguin.model.improved)
```
The F-test indicates that the interactions may be significant.
However, since the 
```{r}
...
Finally we conduct an F-test to compare our improved model with the initial model:
```{r}
#F-Test between improved model and initial model 
anova(penguin.model.improved, penguin.model.initial)
```
With an F-test resulting in a p-value of 0.006, we conclude that our model is
significantly better than both the initial model and Basel's model.
```{r}
confint(penguin.model.improved)
```

An F-test was conducted to check the importance of the interaction term between billdepth and species. This gave a F-value of 5.1625 and a Pr(>F) of 0.006193. Though this might mean that the interaction term between billdepth and species have some significance, it is the least significant and have been chosen to be omitted for simplicity. The final model without the interaction term have low p-values for all included covariates. Based on the dummy variable for Gentoo penguins being the largest (657), and no species interaction term the Gentoo penguins have in general the largest mass. However this is done assuming that the other covariates, bill_depth and flipper_length, are independent of species, which might not the case. The final model is therefore given by the equations.    
